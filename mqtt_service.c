...
/*-----------------------------------------------------------*/
static int mqtt_atop_request_send(void* userdata, const uint8_t* data, size_t datalen)
{
	tuya_mqtt_context_t* context = (tuya_mqtt_context_t*)userdata;

	uint16_t msgid = mqtt_client_publish(context->mqttctx, context->matop_service.resquest_topic, data, datalen, MQTT_QOS_0);
...
static void mqtt_status_handle_on(tuya_iot_client_t *client, tuya_event_id_t id)
{
	client->event.id = id;
...
static void mqtt_client_connected_cb(void* client, void* userdata)
{
	...
	msgid = mqtt_client_subscribe(client, context->signature.topic_in, MQTT_QOS_1);
	if (msgid <= 0) {
		TY_LOGE("subscribe %s fault.", context->signature.topic_in);
		/* reconnect */
		mqtt_client_disconnect(context->mqttctx);
		return;
	}
	TY_LOGD("SUBSCRIBE id:%d sent for topic %s to broker.", msgid, context->signature.topic_in);

	if (context->is_activated) {
		msgid = mqtt_client_subscribe(client, context->matop_service.response_topic, MQTT_QOS_1);
		if (msgid <= 0) {
			TY_LOGE("subscribe %s fault.", context->matop_service.response_topic);
			/* reconnect */
			mqtt_client_disconnect(context->mqttctx);
			return;
		}
		TY_LOGD("SUBSCRIBE id:%d sent for topic %s to broker.", msgid, context->matop_service.response_topic);

		msgid = mqtt_client_subscribe(client, context->matop_service.ota_topic, MQTT_QOS_0);
		if (msgid <= 0) {
			TY_LOGE("subscribe %s fault.", context->matop_service.ota_topic);
			/* reconnect */
			mqtt_client_disconnect(context->mqttctx);
			return;
		}
		TY_LOGD("SUBSCRIBE id:%d sent for topic %s to broker.", msgid, context->matop_service.ota_topic);
	}

	context->is_connected = true;
	mqtt_status_handle_on((tuya_iot_client_t *)context->user_data, TUYA_EVENT_MQTT_CONNECTED);
}
...
static void mqtt_client_disconnected_cb(void* client, void* userdata)
{
	...
	mqtt_status_handle_on((tuya_iot_client_t *)context->user_data, TUYA_EVENT_MQTT_DISCONNECT);
}
...
static void mqtt_client_message_cb(void* client, uint16_t msgid, const mqtt_client_message_t* msg, void* userdata)
{
	...
	} else if (memcmp(msg->topic, context->matop_service.ota_topic, context->matop_service.ota_topic_length) == 0) {
		matop_service_file_rawdata_receive_cb(context->matop, msg->payload, msg->length);

	} else if (memcmp(msg->topic, context->matop_service.response_topic, context->matop_service.response_topic_length) == 0) {
		matop_service_data_receive_cb(context->matop, msg->payload, msg->length);
	}
}
...
int tuya_mqtt_init(tuya_mqtt_context_t* context, const tuya_mqtt_config_t* config)
{
	...
	/* if have devid and seckey, the client is activated. */
	if (config->devid && config->seckey) {
	...
	/* ATOP RPC */
#if MATOP_FEATURE_ENABLE
	if (context->is_activated) {
		context->matop_service.resquest_topic_length = snprintf(context->matop_service.resquest_topic,
			sizeof context->matop_service.resquest_topic, "rpc/req/%s", config->devid);

		context->matop_service.response_topic_length = snprintf(context->matop_service.response_topic,
			sizeof context->matop_service.response_topic, "rpc/rsp/%s", config->devid);

		context->matop_service.ota_topic_length = snprintf(context->matop_service.ota_topic,
			sizeof context->matop_service.ota_topic, "rpc/file/%s", config->devid);

		context->matop = system_malloc(sizeof(matop_context_t));
		if (context->matop == NULL) {
			TY_LOGE("matop_service.ctx malloc fault.");
			return OPRT_MALLOC_FAILED;
		}

		matop_serice_init(context->matop, &(const matop_config_t) {
			.request_send = mqtt_atop_request_send,
			.userdata = context
		});
	}
#endif
...
int tuya_mqtt_stop(tuya_mqtt_context_t* context)
{
	...
	mqtt_client_status_t mqtt_status;
	uint16_t msgid = mqtt_client_unsubscribe(context->mqttctx, context->signature.topic_in, 1);
	if (msgid <= 0) {
...
int tuya_mqtt_loop(tuya_mqtt_context_t* context)
{
	...
	/* reconnect */
	if (context->is_connected == false) {
		if (mqtt_client_connect(context->mqttctx) == MQTT_STATUS_NOT_AUTHORIZED) {
...
#if MATOP_FEATURE_ENABLE
	rt = matop_serice_yield(context->matop);
	if (rt == OPRT_TIMEOUT) {
		mqtt_client_disconnect(context->mqttctx);
	}
#endif
